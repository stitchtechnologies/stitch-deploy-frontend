...
export default function EditService() {
    ...
    const handleCreateServiceClick = async (e: any) => {
        e.preventDefault()
        setSending(true)
        let scriptV2 = {};
            ...
            .then((data) => {
                if (data.service) {
                    toast({
                        ...
                        description: "Your service has been updated. You can now share the installer.",
                        action: (
                            <Link href={`https://deploy.stitch.tech/${service?.slug}/${data.service.slug}`} target="_blank">
                    ...
                    toast({
                        title: "Error",
                        description: data.message ?? "There was an error updating your service. Please try again.",
                    })
                    setSending(false)
                }
            })
    ...
    return (
        <Layout>
            <Head>
                <title>Stitch | Edit Service</title>
            </Head>
            ...
            {
                !loadingService && (
                    <form onSubmit={handleCreateServiceClick}>
    ...
    services:`}
                        ...
                        <div className="flex flex-col gap-6 py-12 px-24 border-b-[rgba(0,0,0,0.10)] border-b border-solid">
                            <Button disabled={disableCreateService} type="submit">{sending ? <><Loader2 className="h-4 w-4 animate-spin mr-2" /> Updating</> : "Edit service"}</Button>
                        </div>
                    </form>
                )
            }
