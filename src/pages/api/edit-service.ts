...
export default async function handler(
  ...
  const { id, name, description, slug, externalUrl, scriptV2, port, validationUrl, imageUrl, environmentVariables, readMe } = req.body;
  ...
  if (!existingService) {
    res.status(400).json({ vendor: undefined, message: "Cannot edit a service which does not exist, please try another name or reach out to ali@stitch.tech" });
    return;
  }
